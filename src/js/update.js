    ...
    if(b.life<=0||solid(b.x,b.y)){
      ...
      if(secretWallPos&&bTx===secretWallPos.tx&&bTy===secretWallPos.ty){
        ...
        if(secretWallHits>=25){
          ...
          pickups.push({type:'driver',x:bTx*TILE+8,y:bTy*TILE+8,t:0});
          flash('DRIVER FOUND!','#ffd700');flash('3-WAY SHOT UNLOCKED','#ffd700');
  ...
  for(let i=pickups.length-1;i>=0;i--){
    ...
    for(const pl of players){if(!pl.alive)continue;const dx=pl.x-p.x,dy=pl.y-p.y;
      if(dx*dx+dy*dy<36){
        ...
        else if(p.type==='driver'){driverActive=true;flash('DRIVER ACTIVE — 3-WAY','#ffd700');spark(pl.x,pl.y,'#ffd700',16,110);SE.driver();}
