...
class ClearAIController{
  ...
  move(p){
    ...
    const boss=enemies.find(e=>e.type==='boss');
    const nearCount=enemies.filter(e=>Math.hypot(e.x-p.x,e.y-p.y)<48).length;
    const lowHP=p.hp<p.maxHp*.3;
    // 0. Emergency: low HP or surrounded → flee
    if((lowHP||nearCount>=3)&&ce){
